...
$defaut2 = 0;
$id = 0;
$name = (int) $_POST['TicketId'];

if ($var == 0)
{
	if ($name==""){
		echo("Please, enter an Id for the ticket." . "<br>");
		$defaut++;
	}	
}
...
if ($defaut == 0)
{
	...
	$sql = "SELECT ticketid FROM panierinfo";
	$result = $mysqli->query($sql);
	...
	if ($defaut2 == 0)
	{
		...
		$sql = "SELECT ticketprice, ticketamount, ticketid FROM panierinfo WHERE `panierinfo`.`ticketid` = $id";
		$result = $mysqli->query($sql);
		...
		if ($result->num_rows > 0) 
		{
			...
			while($row = $result->fetch_assoc())
			{
				$number = $row["ticketamount"];
				$price = $row["ticketprice"];
			}
		}

		$valpanier = $valpanier - ($number*$price);

		$sql = "UPDATE `panier` SET `panierval` = '$valpanier'";
		$result = $mysqli->query($sql);

		$sql = "DELETE FROM `panierinfo` WHERE `panierinfo`.`ticketid` = $id";
		$result = $mysqli->query($sql);
		echo("<br><br><br><br><br>Ticket successfully delete<br><br>");
		?>
...
else
{
	echo("<br><br>");
	?>
	<button><A href="http://localhost/A&H Cinema - PHP/A&H Cinema - Cart.php">Go Back</A></button>
	<?php	
}
?>
...
<title>A&H CINEMA - DELETE A TICKET</title>
</head>
